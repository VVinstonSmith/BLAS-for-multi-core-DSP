...
int main(int argc, char **argv){
	...
	hthread_dat_load(dev_id, "irr_thinB_dgemm_launcher.dat");
	...
	for(int i=0; i<NUM_TESTS; i++){
		hthread_group_exec(tg_id, "launch_irr_thinB_dgemm_null", 4, 3, args_GEMM_TT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	t_ov = t_end - t_start;
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_thinB_dgemm_TT", 4, 3, args_GEMM_TT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_thinB_dgemm_NT", 4, 3, args_GEMM_NT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
		hthread_group_exec(tg_id, "launch_irr_thinB_dgemm_TN", 5, 3, args_GEMM_TN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_thinB_dgemm_NN", 5, 3, args_GEMM_NN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
