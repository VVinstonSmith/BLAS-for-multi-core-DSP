...
__global__ void dgemm_NN(unlong M, unlong N, unlong K, unlong b_id, double* A, double* B, double* C){
    ...
    if(n_start < N){
        // preload spm_B
        ch_bl[0] = dma_p2p_opt(&B[OFFSET(0, n_start, N)], K_G_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double),
                            spm_B[0], K_G_cur, N_A_cur*sizeof(double), (N_A-N_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_bl);
        // preload spm_C
        ch_cl[0] = dma_p2p_opt(&C[OFFSET(0, n_start, N)], M_A_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double),
        ...
        for(long m_o=0; m_o<M; m_o+=M_G){
            ...
            if(tid==0){
                ...
                if(m_o + M_G < M || k_o + K_G < K){
                    ch_gsm[cnt_gsm_1] = dma_p2p_opt(&A[OFFSET(m_o_next, k_o_next, K)], M_G_next, K_G_next*sizeof(double), (K-K_G_next)*sizeof(double),
            ...
            for(long n_o= n_start; n_o<N; n_o+=n_step){
                ...
                if(n_o + n_step < N){ // n_o没到头
                    ...
                    ch_bl[cnt_b_1] = dma_p2p_opt(&B[OFFSET(k_o, n_o+n_step, N)], K_G_cur, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                    ...
                    ch_bl[cnt_b_1] = dma_p2p_opt(&B[OFFSET(k_o_next, n_start, N)], K_G_next, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                ...
                for(long m_i=0; m_i<M_G_cur; m_i+=M_A){
                    ...
                    if(m_i + M_A < M_G_cur){ // m_i没到头
                        ...
                        ch_cl[cnt_c_1] = dma_p2p_opt(&C[OFFSET(m_o+(m_i+M_A), n_o, N)], M_A_next, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double),
                        ...
                        ch_cl[cnt_c_1] = dma_p2p_opt(&C[OFFSET(m_o_next, n_o_next, N)], M_A_next, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                        ...
                        ch_cl[cnt_c_1] = dma_p2p_opt(&C[OFFSET(m_o, n_o_next, N)], M_A_next, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                                            spm_C[cnt_c_1], M_A_next, N_A_next*sizeof(double), (N_A-N_A_next)*sizeof(double), row_syn, p2pmask_next, ch0_cl+cnt_d_1);
                    }
                    ...
                    ch_cs[cnt_c] = dma_p2p_opt(spm_C[cnt_c], M_A_cur, N_A_cur*sizeof(double), (N_A-N_A_cur)*sizeof(double),
                                &C[OFFSET(m_o+m_i, n_o, N)], M_A_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_cs+cnt_d);
...
__global__ void dgemm_NT(unlong M, unlong N, unlong K, unlong b_id, double* A, double* B, double* C){
    ...
    if(n_start < N){
        // preload spm_B
        ch_bl = dma_p2p_opt(&B[OFFSET(n_start, 0, K)], N_A_cur, K_G_cur*sizeof(double), (K-K_G_cur)*sizeof(double),
        ...
        for(long m_o=0; m_o<M; m_o+=M_G){
            ...
            for(long n_o= n_start; n_o<N; n_o+=n_step){
                ...
                if(n_o + n_step < N){ // n_o没到头
                    n_o_next = n_o + n_step;
                    N_A_next = min(N_A, N-(n_o+n_step));
                    ch_bl = dma_p2p_opt(&B[OFFSET(n_o+n_step, k_o, K)], N_A_next, K_G_cur*sizeof(double), (K-K_G_cur)*sizeof(double),
                                spm_B[0], N_A_next, K_G_cur*sizeof(double), (K_G-K_G_cur)*sizeof(double), row_syn, p2pmask_next, ch0_bl);
                }else if(m_o_next != -1){ // n_o到头了，m_o,k_o还没到头
                    n_o_next = n_start;
                    N_A_next = min(N_A, N-n_start);
                    ch_bl = dma_p2p_opt(&B[OFFSET(n_start, k_o_next, K)], N_A_next, K_G_cur*sizeof(double), (K-K_G_cur)*sizeof(double),
                                spm_B[0], N_A_next, K_G_cur*sizeof(double), (K_G-K_G_cur)*sizeof(double), row_syn, p2pmask_next, ch0_bl);
                }else{ // n_o到头了，m_o,k_o也到头了
                    n_o_next = -1;
                }
...
void transpose_gsm_16x16_dbuf(unlong M, unlong K, unlong bid_main, unlong bid_T, int n_threads, double* A){ // n_threads:多少个核参与转置操作
    const int tid = get_thread_id();
    ...
    for(long k_o=0; k_o<K; k_o+=K_G){
        ...
        for(long m_o=0; m_o<M; m_o+=M_G){
            ...
            if(tid == tid_0){
                ch_gsm[cnt_dgsm] = dma_p2p_opt(&A[OFFSET(k_o, m_o, M)], K_G_cur, M_G_cur*sizeof(double), (M-M_G_cur)*sizeof(double),
...
void dgemm_TN_gsmA_tribuf(unlong M, unlong N, unlong K, unlong b_id, const int grp_size,
...
__global__ void dgemm_TN(unlong M, unlong N, unlong K, unlong bid_main, unlong bid_T, double* A, double* B, double* C){
    ...
    if(tid >= n_cores_for_compute){
        transpose_gsm_16x16_dbuf(M, K, bid_main, bid_T, n_cores_for_transpose, A);
    }else{
        dgemm_TN_gsmA_tribuf(M, N, K, bid_main, n_cores_for_compute, A, B, C);
    }
}
...
__global__ void dgemm_TT(unlong M, unlong N, unlong K, unlong b_id, double* A, double* B, double* C){
    ...
    if(m_start < M){
        // preload spm_A
        ch_al[0] = dma_p2p_opt(&A[OFFSET(0, m_start, M)], K_G_cur, M_A_cur*sizeof(double), (M-M_A_cur)*sizeof(double),
                            spm_A[0], K_G_cur, M_A_cur*sizeof(double), (M_A-M_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_al);
        // preload spm_C
        ch_cl[0] = dma_p2p_opt(&C[OFFSET(m_start, 0, N)], M_A_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double),
        ...
        for(long n_o=0; n_o<N; n_o+=N_G){
            ...
            if(tid==0){
                ...
                if(n_o + N_G < N || k_o + K_G < K){
                    ch_gsm[cnt_gsm_1] = dma_p2p_opt(&B[OFFSET(n_o_next, k_o_next, K)], N_G_next, K_G_next*sizeof(double), (K-K_G_next)*sizeof(double),
            ...
            for(long m_o=m_start; m_o<M; m_o+=m_step){
                ...
                if(m_o + m_step < M){ // m_o没到头
                    ...
                    ch_al[cnt_a_1] = dma_p2p_opt(&A[OFFSET(k_o, m_o+m_step, M)], K_G_cur, M_A_next*sizeof(double), (M-M_A_next)*sizeof(double),
                    ...
                    ch_al[cnt_a_1] = dma_p2p_opt(&A[OFFSET(k_o_next, m_start, M)], K_G_next, M_A_next*sizeof(double), (M-M_A_next)*sizeof(double),
                ...
                for(long n_i=0; n_i<N_G_cur; n_i+=N_A){
                    ...
                    if(n_i + N_A < N_G_cur){ // n_i没到头
                        ...
                        ch_cl[cnt_c_1] = dma_p2p_opt(&C[OFFSET(m_o, n_o+(n_i+N_A), N)], M_A_cur, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                        ...
                        ch_cl[cnt_c_1] = dma_p2p_opt(&C[OFFSET(m_o_next, n_o, N)], M_A_next, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                                            spm_C[cnt_c_1], M_A_next, N_A_next*sizeof(double), (N_A_buffer-N_A_next)*sizeof(double), row_syn, p2pmask_next, ch0_cl+cnt_c_1);
                    }

                    // [5] store spm_C to C
                    ch_cs[cnt_c] = dma_p2p_opt(spm_C[cnt_c], M_A_cur, N_A_cur*sizeof(double), (N_A_buffer-N_A_cur)*sizeof(double),
                                &C[OFFSET(m_o, n_o+n_i, N)], M_A_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_cs+cnt_c);
...
__global__ void  dgemm_null(unlong M, unlong N, unlong K, 
