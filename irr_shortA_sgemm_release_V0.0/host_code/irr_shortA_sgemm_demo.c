...
int main(int argc, char **argv){
	...
	hthread_dat_load(dev_id, "irr_shortA_sgemm_launcher.dat");
	...
	unsigned long args_GEMM_TT[8];
	args_GEMM_TT[0] = M; args_GEMM_TT[1] = N; args_GEMM_TT[2] = K;
	args_GEMM_TT[3] = bid_main; args_GEMM_TT[4] = bid_T; 
	args_GEMM_TT[5] = (unsigned long)A_T;
	args_GEMM_TT[6] = (unsigned long)B_T;
	args_GEMM_TT[7] = (unsigned long)C_TT;	
	//--------------------------------------------------------------------------------------
	/*在指定的DSP簇上创建空闲线程组*/
	...
	for(int i=0; i<NUM_TESTS; i++){
		hthread_group_exec(tg_id, "launch_irr_shortA_sgemm_null", 4, 3, args_GEMM_NN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	t_ov = t_end - t_start;
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_shortA_sgemm_NN", 4, 3, args_GEMM_NN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_shortA_sgemm_NT", 4, 3, args_GEMM_NT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
		hthread_group_exec(tg_id, "launch_irr_shortA_sgemm_TN", 5, 3, args_GEMM_TN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_shortA_sgemm_TT", 5, 3, args_GEMM_TT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
