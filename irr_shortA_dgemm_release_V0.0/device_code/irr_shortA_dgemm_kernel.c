...
// traverse order: M_G -> K_G -> N_A -> K_A -> M_S
...
__global__ void irr_shortA_dgemm_NN_dbuf(unlong M, unlong N, unlong K, unlong b_id, double* A, double* B, double* C){
    ...
    spm_A[0] = scalar_malloc(M_S * K_A *sizeof(double));
    spm_A[1] = scalar_malloc(M_S * K_A *sizeof(double));
    spm_B[0] = vector_malloc(K_A * N_A * sizeof(double));
    spm_B[1] = vector_malloc(K_A * N_A * sizeof(double));
    ...
    long M_G_next, K_G_next, N_A_next, K_A_next;

    // preload A to gsm_mem
    if(tid==0){
        ch_gsm[0] = dma_p2p_opt(&A[OFFSET(0, 0, K)], M_G_cur, K_G_cur*sizeof(double), (K-K_G_cur)*sizeof(double),
                            gsm_mem, M_G_cur, K_G_cur*sizeof(double), (K_G-K_G_cur)*sizeof(double), 0, 0, ch0_gsm);
    }
    if(c_start < N){
        // preload spm_C
        ch_cl[0] = dma_p2p_opt(&C[OFFSET(0, c_start, N)], M_G_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double),
                            spm_C[0], M_G_cur, N_A_cur*sizeof(double), (N_A-N_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_cl);
        // preload spm_B
        ch_bl[0] = dma_p2p_opt(&B[OFFSET(0, c_start, N)], K_A_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double),
                            spm_B[0], K_A_cur, N_A_cur*sizeof(double), (N_A-N_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_bl);        
    }
    int cnt_gsm = 0;
    int cnt_b = 0;
    int cnt_c = 0;
        ...
        for(long k_o=0; k_o<K; k_o+=K_G){
            ...
            if(tid==0){
                ...
                if(k_o + K_G < K){ // not row end
                    ...
                    ch_gsm[cnt_gsm_1] = dma_p2p_opt(&A[OFFSET(r, k_o_next, K)], M_G_next, K_G_next*sizeof(double), (K-K_G_next)*sizeof(double),
                                            gsm_mem + MK_G*cnt_gsm_1, M_G_next, K_G_next*sizeof(double), (K_G-K_G_next)*sizeof(double), 0, 0, ch0_gsm+cnt_gsm_1);
                }else if(r + M_G < M){ // k + K_G >= K
                    K_G_next = min(K_G, K);
                    M_G_next = min(M_G, M-(r+M_G));
                    ch_gsm[cnt_gsm_1] = dma_p2p_opt(&A[OFFSET(r_next, 0, K)], M_G_next, K_G_next*sizeof(double), (K-K_G_next)*sizeof(double),
                                            gsm_mem + MK_G*cnt_gsm_1, M_G_next, K_G_next*sizeof(double), (K_G-K_G_next)*sizeof(double), 0, 0, ch0_gsm+cnt_gsm_1);
                }
            ...
            if(c_start<N){
                ch_al[0] = dma_p2p_opt(gsm_mem + MK_G*cnt_gsm, M_S, K_A_cur*sizeof(double), (K_G-K_A_cur)*sizeof(double),
                                    spm_A[0], M_S, K_A_cur*sizeof(double), (K_A-K_A_cur)*sizeof(double), 0, 0, ch0_al);
            }
            unint cnt_a = 0;
            // hthread_printf("M_G_cur=%ld, K_G_cur=%ld\n", M_G_cur, K_G_cur);
            ...
            for(long c=c_start; c<N; c+=c_step){
                ...
                if(c + c_step < N){ // c没到头
                    c_next = c + c_step;
                }else if(r_next == -1){ // c到头了，r,k_o也到头了
                    c_next = -1;
                }else{ // c到头了，r,k_o还没到头
                    c_next = c_start;
                }
                ...
                if(c + c_step < N){ // c没到头
                    N_A_next = min(N_A, N-(c+c_step));
                    ch_cl[cnt_c_1] = dma_p2p_opt(&C[OFFSET(r, c_next, N)], M_G_cur, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                                        spm_C[cnt_c_1], M_G_cur, N_A_next*sizeof(double), (N_A-N_A_next)*sizeof(double), row_syn, p2pmask_next, ch0_cl+cnt_c_1);
                }else if(c_next != -1){ // c到头了，r,k_o没到头
                    N_A_next = min(N_A, N-c_start);
                    ch_cl[cnt_c_1] = dma_p2p_opt(&C[OFFSET(r_next, c_next, N)], M_G_next, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                                        spm_C[cnt_c_1], M_G_next, N_A_next*sizeof(double), (N_A-N_A_next)*sizeof(double), row_syn, p2pmask_next, ch0_cl+cnt_c_1);
                }
                // wait for am_C_cur
                ...
                for(long k_i=0; k_i<K_G_cur; k_i+=K_A){    
                    ...
                    if(k_i + K_A < K_G_cur){ // k_i没到头
                        ...
                        ch_bl[cnt_b_1] = dma_p2p_opt(&B[OFFSET(k_o+k_i+K_A, c, N)], K_A_next, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double),
                                                spm_B[cnt_b_1], K_A_next, N_A_cur*sizeof(double), (N_A-N_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_bl+cnt_b_1);
                    }else if(c + c_step < N){ // k_i到头了，c没到头
                        K_A_next = min(K_A, K_G_cur);
                        ch_bl[cnt_b_1] = dma_p2p_opt(&B[OFFSET(k_o, c+c_step, N)], K_A_next, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                                                spm_B[cnt_b_1], K_A_next, N_A_next*sizeof(double), (N_A-N_A_next)*sizeof(double), row_syn, p2pmask_next, ch0_bl+cnt_b_1);
                    }else if(r_next != -1){ // k_i到头了，c到头了，m_o,k_o还没到头
                        K_A_next = min(K_A, K_G_next);
                        ch_bl[cnt_b_1] = dma_p2p_opt(&B[OFFSET(k_o_next, c_start, N)], K_A_next, N_A_next*sizeof(double), (N-N_A_next)*sizeof(double),
                                                spm_B[cnt_b_1], K_A_next, N_A_next*sizeof(double), (N_A-N_A_next)*sizeof(double), row_syn, p2pmask_next, ch0_bl+cnt_b_1);
                    }
                    // wait for spm_B[cnt_b]
                    dma_wait_p2p(ch_bl[cnt_b]);

                    for(int ra=0; ra<M_G_cur; ra+=2*M_S){
                        // preload A to sm
                        ch_al[1] = dma_p2p_opt(gsm_mem + MK_G*cnt_gsm + (ra+M_S)*K_G + k_i, M_S, K_A_cur*sizeof(double), (K_G-K_A_cur)*sizeof(double),
                                            spm_A[1], M_S, K_A_cur*sizeof(double), (K_A-K_A_cur)*sizeof(double), 0, 0, ch0_al+1);
                        dma_wait_p2p(ch_al[0]);
                        micro_kernel_asm_r6c64(spm_A[0], spm_B[cnt_b], spm_C[cnt_c] + ra*(N_A>>4), K_A_cur, K_A, N_A);

                        // preload A to sm
                        if(ra + 2*M_S < M_G_cur){ // ra没到头
                            ch_al[0] = dma_p2p_opt(gsm_mem + MK_G*cnt_gsm + (ra+2*M_S)*K_G + k_i, M_S, K_A_cur*sizeof(double), (K_G-K_A_cur)*sizeof(double),
                                                spm_A[0], M_S, K_A_cur*sizeof(double), (K_A-K_A_cur)*sizeof(double), 0, 0, ch0_al+0);
                        }else if(k_i + K_A < K_G_cur || c + c_step < N){ // ra到头了，k_i或c没到头
                            ch_al[0] = dma_p2p_opt(gsm_mem + MK_G*cnt_gsm + k_i_next, M_S, K_A_next*sizeof(double), (K_G-K_A_next)*sizeof(double),
                                                spm_A[0], M_S, K_A_next*sizeof(double), (K_A-K_A_next)*sizeof(double), 0, 0, ch0_al+0);
                        }
                        dma_wait_p2p(ch_al[1]);
                        micro_kernel_asm_r6c64(spm_A[1], spm_B[cnt_b], spm_C[cnt_c] + (ra+M_S)*(N_A>>4), K_A_cur, K_A, N_A);
                    }
                    // renew index B
                    cnt_b = cnt_b_1;
                    K_A_cur = K_A_next;
                ...
                // store spm_C to C
                ch_cs = dma_p2p_opt(spm_C[cnt_c], M_G_cur, N_A_cur*sizeof(double), (N_A-N_A_cur)*sizeof(double),
                            &C[OFFSET(r, c, N)], M_G_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_cs);
...
__global__ void irr_shortA_dgemm_NT_dbuf(unlong M, unlong N, unlong K, unlong b_id, double* A, double* B, double* C){
    ...
    if(c_start < N){
        ...
        ch_cl[0] = dma_p2p_opt(&C[OFFSET(0, c_start, N)], M_G_cur, N_A_cur*sizeof(double), (N-N_A_cur)*sizeof(double),
                            spm_C[0], M_G_cur, N_A_cur*sizeof(double), (N_A-N_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_cl);
        // preload spm_B
        ch_bl = dma_p2p_opt(&B[OFFSET(c_start, 0, K)], N_A_cur, K_A_cur*sizeof(double), (K-K_A_cur)*sizeof(double),
                            spm_B[0], N_A_cur, K_A_cur*sizeof(double), (K_A-K_A_cur)*sizeof(double), row_syn, p2pmask_cur, ch0_bl);        
    }
    int cnt_gsm = 0;
    int cnt_b = 0;
    int cnt_c = 0;
                ...
                for(long k_i=0; k_i<K_G_cur; k_i+=K_A){    
                    ...
                    for(int n_a=0; n_a<N_A_cur; n_a+=16){
                        for(int k_a=0; k_a<K_A_cur; k_a+=32){
                            micro_kernel_16x16x2(spm_B[0] + n_a*(K_A>>4) + (k_a>>4), spm_B[1] + k_a*(N_A>>4) + (n_a>>4), spm_D, N_A, K_A);
                        }
                    }
                    ...
                    if(k_i + K_A < K_G_cur){ // k_i没到头
                        K_A_next = min(K_A, K_G_cur-(k_i+K_A));
                        ch_bl = dma_p2p_opt(&B[OFFSET(c, k_o+k_i+K_A, K)], N_A_cur, K_A_next*sizeof(double), (K-K_A_next)*sizeof(double),
                                                spm_B[0], N_A_cur, K_A_next*sizeof(double), (K_A-K_A_next)*sizeof(double), row_syn, p2pmask_cur, ch0_bl);
                    }else if(c + c_step < N){ // k_i到头了，c没到头
                        K_A_next = min(K_A, K_G_cur);
                        ch_bl = dma_p2p_opt(&B[OFFSET(c+c_step, k_o, K)], N_A_next, K_A_next*sizeof(double), (K-K_A_next)*sizeof(double),
                                                spm_B[0], N_A_next, K_A_next*sizeof(double), (K_A-K_A_next)*sizeof(double), row_syn, p2pmask_next, ch0_bl);
                    }else if(r_next != -1){ // k_i到头了，c到头了，m_o,k_o还没到头
                        K_A_next = min(K_A, K_G_next);
                        ch_bl = dma_p2p_opt(&B[OFFSET(c_start, k_o_next, K)], N_A_next, K_A_next*sizeof(double), (K-K_A_next)*sizeof(double),
                                                spm_B[0], N_A_next, K_A_next*sizeof(double), (K_A-K_A_next)*sizeof(double), row_syn, p2pmask_next, ch0_bl);
                    }
                    ...
                    for(int ra=0; ra<M_G_cur; ra+=2*M_S){
                        ...
                        ch_al[1] = dma_p2p_opt(gsm_mem + MK_G*cnt_gsm + (ra+M_S)*K_G + k_i, M_S, K_A_cur*sizeof(double), (K_G-K_A_cur)*sizeof(double),
                                            spm_A[1], M_S, K_A_cur*sizeof(double), (K_A-K_A_cur)*sizeof(double), 0, 0, ch0_al+1);
                        dma_wait_p2p(ch_al[0]);
                        micro_kernel_asm_r6c64(spm_A[0], spm_B[1], spm_C[cnt_c] + ra*(N_A>>4), K_A_cur, K_A, N_A);
                        ...
                        dma_wait_p2p(ch_al[1]);
                        micro_kernel_asm_r6c64(spm_A[1], spm_B[1], spm_C[cnt_c] + (ra+M_S)*(N_A>>4), K_A_cur, K_A, N_A);
                    }
                    // renew index B
                    cnt_b = cnt_b_1;
                    K_A_cur = K_A_next;
...
void transpose_gsm_16x16(unlong M, unlong K, unlong bid_main, unlong bid_T, int n_threads, double* A){ // n_threads:多少个核参与转置操作
    const int tid = get_thread_id();
    const int tid_0 = get_group_size() - n_threads;
    // --------- GEMM blocking parameters ---------
    const unlong K_G = 2048;
    const unlong M_G = 192;
    ...
    // hthread_printf("tid = %d, m_start = %ld\n", tid, m_start);
    // ---------------------------------------
    lvector double* spm_A[2];
    lvector double* spm_B[2];
    spm_A[0] = vector_malloc(M_A * N_A * sizeof(double));
    ...
    for(long m_o=0; m_o<M; m_o+=M_G){
        ...
        for(long k_o=0; k_o<K; k_o+=K_G){
            ...
            if(tid == tid_0){
                ch_gsm = dma_p2p_opt(&A[OFFSET(k_o, m_o, M)], K_G_cur, M_G_cur*sizeof(double), (M-M_G_cur)*sizeof(double),
                                gsm_mem, K_G_cur, M_G_cur*sizeof(double), (M_G-M_G_cur)*sizeof(double), 0, 0, ch0_gsm);
                dma_wait_p2p(ch_gsm);
            }
            ...
            if(m_start < K_G_cur){
                ch_al[0] = dma_p2p_opt(gsm_mem + OFFSET(m_start, 0, M_G), M_A_cur, N_A_cur*sizeof(double), (M_G-N_A_cur)*sizeof(double),
            ...
            for(unlong m_i=m_start; m_i<K_G_cur; m_i+=m_step){
                unlong M_A_next;
                for(unlong n_i=0; n_i<M_G_cur; n_i+=N_A){
                    // -------------------------- next round indexes --------------------------
                    unlong N_A_next;
                    unint cnt_a_1 = (cnt_a + 1) % 2;
                    unint cnt_b_1 = (cnt_b + 1) % 2;
                    ...
                    if(n_i + N_A < M_G_cur){ // n_i没到头
                        N_A_next = min(N_A, M_G_cur-(n_i+N_A));
                        ch_al[cnt_a_1] = dma_p2p_opt(gsm_mem + OFFSET(m_i, n_i+N_A, M_G), M_A_cur, N_A_next*sizeof(double), (M_G-N_A_next)*sizeof(double),
                        ...
                        ch_al[cnt_a_1] = dma_p2p_opt(gsm_mem + OFFSET(m_i+m_step, 0, M_G), M_A_next, N_A_next*sizeof(double), (M_G-N_A_next)*sizeof(double),
                    ...
                    ch_bs[cnt_b] = dma_p2p_opt(spm_B[cnt_b], N_A_cur, M_A_cur*sizeof(double), (M_A-M_A_cur)*sizeof(double),
                                    gsm_mem + MK_G + OFFSET(n_i, m_i, K_G), N_A_cur, M_A_cur*sizeof(double), (K_G-M_A_cur)*sizeof(double), 0, 0, ch0_bs+cnt_b);
...
// traverse order: M_G -> K_G -> N_A -> K_A -> M_S
// to be ensured: M_G == M_A
__global__ void irr_shortA_dgemm_TN_dbuf_mainbody(unlong M, unlong N, unlong K, unlong b_id, const int grp_size,
    ...
    for(long r=0; r<M; r+=M_G){
        ...
        for(long k_o=0; k_o<K; k_o+=K_G){
            ...
            // preload gsm_mem to sm_A
            if(c_start<N){
                ch_al[0] = dma_p2p_opt(gsm_mem + MK_G, M_S, K_A_cur*sizeof(double), (K_G-K_A_cur)*sizeof(double),
                                    spm_A[0], M_S, K_A_cur*sizeof(double), (K_A-K_A_cur)*sizeof(double), 0, 0, ch0_al);
            }
            unint cnt_a = 0;
            ...
            for(long c=c_start; c<N; c+=c_step){
                ...
                for(long k_i=0; k_i<K_G_cur; k_i+=K_A){    
                    ...
                    for(int ra=0; ra<M_G_cur; ra+=2*M_S){
                        // preload A to sm
                        ch_al[1] = dma_p2p_opt(gsm_mem + MK_G + (ra+M_S)*K_G + k_i, M_S, K_A_cur*sizeof(double), (K_G-K_A_cur)*sizeof(double),
                                            spm_A[1], M_S, K_A_cur*sizeof(double), (K_A-K_A_cur)*sizeof(double), 0, 0, ch0_al+1);
                        dma_wait_p2p(ch_al[0]);
                        micro_kernel_asm_r6c64(spm_A[0], spm_B[cnt_b], spm_C[cnt_c] + ra*(N_A>>4), K_A_cur, K_A, N_A);
                        // preload A to sm
                        if(ra + 2*M_S < M_G_cur){ // ra没到头
                            ch_al[0] = dma_p2p_opt(gsm_mem + MK_G + (ra+2*M_S)*K_G + k_i, M_S, K_A_cur*sizeof(double), (K_G-K_A_cur)*sizeof(double),
                                                spm_A[0], M_S, K_A_cur*sizeof(double), (K_A-K_A_cur)*sizeof(double), 0, 0, ch0_al+0);
                        }else if(k_i + K_A < K_G_cur || c + c_step < N){ // ra到头了，k_i或c没到头
                            ch_al[0] = dma_p2p_opt(gsm_mem + MK_G + k_i_next, M_S, K_A_next*sizeof(double), (K_G-K_A_next)*sizeof(double),
                                                spm_A[0], M_S, K_A_next*sizeof(double), (K_A-K_A_next)*sizeof(double), 0, 0, ch0_al+0);
                        }
                        dma_wait_p2p(ch_al[1]);
                        micro_kernel_asm_r6c64(spm_A[1], spm_B[cnt_b], spm_C[cnt_c] + (ra+M_S)*(N_A>>4), K_A_cur, K_A, N_A);
...
__global__ void irr_shortA_dgemm_TN_dbuf(unlong M, unlong N, unlong K, unlong bid_main, unlong bid_T, double* A, double* B, double* C){
    ...
    if(tid >= n_cores_for_compute){
        transpose_gsm_16x16(M, K, bid_main, bid_T, n_cores_for_transpose, A);
    }else{
        irr_shortA_dgemm_TN_dbuf_mainbody(M, N, K, bid_main, n_cores_for_compute, A, B, C);
    }
}
...
// traverse order: M_G -> K_G -> N_A -> K_A -> M_S
// to be ensured: M_G == M_A
__global__ void irr_shortA_dgemm_TT_dbuf_mainbody(unlong M, unlong N, unlong K, unlong b_id, const int grp_size,
                                double* A, double* B, double* C){
    const int tid = get_thread_id();
    // --------------------------- blocking params ---------------------------
    const indexType M_G = 192;
...
__global__ void irr_shortA_dgemm_TT_dbuf(unlong M, unlong N, unlong K, unlong bid_main, unlong bid_T, double* A, double* B, double* C){
    ...
    if(tid >= n_cores_for_compute){
        ...
        irr_shortA_dgemm_TT_dbuf_mainbody(M, N, K, bid_main, n_cores_for_compute, A, B, C);
    }
}



__global__ void irr_shortA_dgemm_null(unlong M, unlong N, unlong K, 
