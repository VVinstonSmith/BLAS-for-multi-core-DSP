...
int main(int argc, char **argv){
	...
	hthread_dat_load(dev_id, "sgemm_launcher.dat");
    ...
    for(int i=0; i<M*N; i++){
        C_NN[i] = C_NT[i] = C_TN[i] = C_TT[i] = 999.0;
    }
	memset(C_cpu, 0x00, M*N*sizeof(dataType));
	// memset(C_NN, 0x00, M*N*sizeof(dataType));
	// memset(C_NT, 0x00, M*N*sizeof(dataType));
	// memset(C_TN, 0x00, M*N*sizeof(dataType));
	// memset(C_TT, 0x00, M*N*sizeof(dataType));
	...
	for(int i=0; i<NUM_TESTS; i++){
		hthread_group_exec(tg_id, "launch_sgemm_null", 4, 3, args_GEMM_NN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	t_ov = t_end - t_start;
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_sgemm_NN", 4, 3, args_GEMM_NN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_sgemm_NT", 4, 3, args_GEMM_NT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_sgemm_TT", 4, 3, args_GEMM_TT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
		hthread_group_exec(tg_id, "launch_sgemm_TN", 5, 3, args_GEMM_TN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	for(int t=0; t<NUM_TESTS; t++){
		...
		for(int i=0; i<M; i++){
			for(int j=0; j<N; j++){
				...
				// }
				// C_cpu[OFFSET(i, j, N)] += alpha * tmp + beta;

				for(long k=0; k<K-K%K_G; k+=K_G){
					dataType tmp = 0.0;
					for(long kk=0; kk<K_G; kk++){
						tmp = fma(alpha * A[OFFSET(i, k+kk, K)], B[OFFSET(k+kk, j, N)], tmp);
						// tmp = fma(A[OFFSET(i, k+kk, K)], alpha * B[OFFSET(k+kk, j, N)], tmp);
					}
					C_cpu[OFFSET(i, j, N)] += tmp;
				}
				dataType tmp = 0.0;
				for(long k=K-K%K_G; k<K; k++){
					tmp = fma(alpha * A[OFFSET(i, k, K)], B[OFFSET(k, j, N)], tmp);
					// tmp = fma(A[OFFSET(i, k, K)], alpha * B[OFFSET(k, j, N)], tmp);
				}
				C_cpu[OFFSET(i, j, N)] += tmp;
				C_cpu[OFFSET(i, j, N)] += beta;
			}
		}
	}
	double Sum_cpu = 0.0;
