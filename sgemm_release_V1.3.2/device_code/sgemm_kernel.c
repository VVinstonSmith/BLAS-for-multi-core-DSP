...
__global__ void sgemm_NN_qxx(unlong M, unlong N, unlong K, unlong b_id, float* A, float* B, float* C, float alpha, float beta){
    ...
    for(long r=0; r<M; r+=M_G){
        ...
        for(long c=c_start; c<N; c+=c_step){ 
            ...
            #ifdef use_r6c96
                    micro_kernel_asm_r6c96_only_write(spm_A[0], spm_B[cnt_b], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c96_only_write(spm_A[0] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
            #endif
            #ifdef use_r6c128
                #ifndef enable_alpha
                    micro_kernel_asm_r6c128_only_write(spm_A[0], spm_B[cnt_b], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c128_only_write(spm_A[0] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
                #else
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[0], spm_B[cnt_b], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[0] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                #endif
            #endif
                    // preload A to sm
            ...
            #ifdef use_r6c96
                    micro_kernel_asm_r6c96_only_write(spm_A[1], spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S)*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c96_only_write(spm_A[1] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
            #endif
            #ifdef use_r6c128
                #ifndef enable_alpha
                    micro_kernel_asm_r6c128_only_write(spm_A[1], spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S)*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c128_only_write(spm_A[1] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
                #else
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[1], spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S)*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[1] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                #endif
            #endif
                }
#else
            ...
            #ifdef use_r6c96
                    micro_kernel_asm_r6c96_only_write(spm_A[cnt_a], spm_B[cnt_b], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c96_only_write(spm_A[cnt_a] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
            #endif
            #ifdef use_r6c128
                #ifndef enable_alpha
                    micro_kernel_asm_r6c128_only_write(spm_A[cnt_a], spm_B[cnt_b], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c128_only_write(spm_A[cnt_a] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
                #else
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[cnt_a], spm_B[cnt_b], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[cnt_a] + M_S_half*K_G, spm_B[cnt_b], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                #endif
            #endif
                    cnt_a = cnt_a_1;
                }
        ...
        for(long c=c_start; c<N; c+=c_step){
            ...
            for(int rc=0; rc<M_G_cur; rc+=M_A){
                ...
                for(int ra=0; ra<M_A_cur; ra+=2*M_S){
                    ...
                    dma_wait_p2p(ch_al[0]);
            #ifdef use_r6c96
                    micro_kernel_asm_r6c96_only_write(spm_A[0], spm_B[1], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c96_only_write(spm_A[0] + M_S_half*K_G, spm_B[1], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
            #endif
            #ifdef use_r6c128
                #ifndef enable_alpha
                    micro_kernel_asm_r6c128_only_write(spm_A[0], spm_B[1], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c128_only_write(spm_A[0] + M_S_half*K_G, spm_B[1], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
                #else
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[0], spm_B[1], spm_C[cnt_c] + ra*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[0] + M_S_half*K_G, spm_B[1], spm_C[cnt_c] + (ra + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                #endif
            #endif
                    // preload A to sm
                    if(ra + 2*M_S < M_A_cur){ // ra没到头
                    ...
                    dma_wait_p2p(ch_al[1]);
            #ifdef use_r6c96
                    micro_kernel_asm_r6c96_only_write(spm_A[1], spm_B[1], spm_C[cnt_c] + (ra + M_S)*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c96_only_write(spm_A[1] + M_S_half*K_G, spm_B[1], spm_C[cnt_c] + (ra + M_S + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
            #endif
            #ifdef use_r6c128
                #ifndef enable_alpha
                    micro_kernel_asm_r6c128_only_write(spm_A[1], spm_B[1], spm_C[cnt_c] + (ra + M_S)*(N_A>>5), K_G_cur, K_G, N_A);
                    micro_kernel_asm_r6c128_only_write(spm_A[1] + M_S_half*K_G, spm_B[1], spm_C[cnt_c] + (ra + M_S + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A);
                #else
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[1], spm_B[1], spm_C[cnt_c] + (ra + M_S)*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                    micro_kernel_asm_r6c128_multiAlpha_only_write(spm_A[1] + M_S_half*K_G, spm_B[1], spm_C[cnt_c] + (ra + M_S + M_S_half)*(N_A>>5), K_G_cur, K_G, N_A, alpha);
                #endif
            #endif
                }
                // last_C plus beta
            ...
            for(int cc=0; cc<N_G_cur; cc+=N_A){
                ...
                for(int n_a=0; n_a<N_A_cur; n_a+=32){
                    for(int m_a=0; m_a<M_A_cur; m_a+=32){
                        micro_kernel_float_32x32(&spm_C[2][OFFSET(n_a, m_a>>5, M_A>>5)], &spm_C[cnt_c][OFFSET(m_a, n_a>>5, N_A_buffer>>5)], spm_D, N_A_buffer, M_A);
                    }
                }
                // last_C plus beta
            #ifdef enable_beta
                ...
                }else if(r_next == r_start){ // cc到头了，r也到头了，c,k还没到头
                    N_A_next = min(N_A, N_G_next);
                    if(k_next != 0){ // 如果下一个block_B在第二行
                        ch_cl[cnt_c_1] = dma_p2p_opt(&C[OFFSET(r_next, c_next, N)], M_A_next, N_A_next*sizeof(float), (N-N_A_next)*sizeof(float),
