...
int main(int argc, char **argv){
	...
	hthread_dat_load(dev_id, "irr_thinB_sgemm_launcher.dat");
	...
	for(int i=0; i<M; i++){
		for(int j=0; j<K; j++){
			// A[OFFSET(i, j, K)] = A_T[OFFSET(j, i, M)] = 1.0;
			// A[OFFSET(i, j, K)] = A_T[OFFSET(j, i, M)] = (float)(rand()) / RAND_MAX * (-4.0 + 6.0) + 2.0;
	...
	for(int i=0; i<K; i++){
		for(int j=0; j<N; j++){
			// B[OFFSET(i, j, N)] = B_T[OFFSET(j, i, K)] = 1.0;
			// B[OFFSET(i, j, N)] = B_T[OFFSET(j, i, K)] = (float)(rand()) / RAND_MAX * (-4.0 + 6.0) + 2.0;
    ...
    for(int i=0; i<M*N; i++){
        C_cpu[i] = 0.0;
		C_NN[i] = C_NT[i] = C_TN[i] = C_TT[i] = 999;
    }
	...
	unsigned long args_GEMM_NN[8];
	args_GEMM_NN[0] = M; args_GEMM_NN[1] = N; args_GEMM_NN[2] = K;
	args_GEMM_NN[3] = bid_main; args_GEMM_NN[4] = bid_T; 
	args_GEMM_NN[5] = (unsigned long)A;
	args_GEMM_NN[6] = (unsigned long)B;
	args_GEMM_NN[7] = (unsigned long)C_NN;
	//--------------------------------------------------------------------------------------
	/*在指定的DSP簇上创建空闲线程组*/
	...
	for(int i=0; i<NUM_TESTS; i++){
		hthread_group_exec(tg_id, "launch_irr_thinB_sgemm_null", 4, 3, args_GEMM_TT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	t_ov = t_end - t_start;
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_thinB_sgemm_TT", 4, 3, args_GEMM_TT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_thinB_sgemm_NT", 4, 3, args_GEMM_NT);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
		hthread_group_exec(tg_id, "launch_irr_thinB_sgemm_TN", 5, 3, args_GEMM_TN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	t_start = timer();
	for(int i=0; i<NUM_TESTS; i++){
        hthread_group_exec(tg_id, "launch_irr_thinB_sgemm_NN", 5, 3, args_GEMM_NN);
		hthread_group_wait(tg_id);
	}
	t_end = timer();
	...
	for(int t=0; t<NUM_TESTS; t++){
		...
		for(int i=0; i<M; i++){
			for(int j=0; j<N; j++){
				dataType tmp_o = 0.0;
				for(int k_o=0; k_o<K; k_o+=K_G){
					int K_G_cur = min(K_G, K-k_o);
					dataType tmp_i = 0;
					for(int k_i=0; k_i<K_G_cur; k_i+=K_A){
						int K_A_cur = min(K_A, K_G_cur-k_i);
						dataType tmp_s = 0;
						for(int k_s=0; k_s<K_A_cur; k_s++){
							int k_index = k_o + k_i + k_s;
							tmp_s = fma(A[OFFSET(i, k_index, K)], alpha * B[OFFSET(k_index, j, N)], tmp_s);
						}
						tmp_i += tmp_s;
					}
					tmp_o += tmp_i;
				}
				C_cpu[OFFSET(i, j, N)] += tmp_o + beta;
			}
		}
	}
	double Sum_cpu = 0.0;
