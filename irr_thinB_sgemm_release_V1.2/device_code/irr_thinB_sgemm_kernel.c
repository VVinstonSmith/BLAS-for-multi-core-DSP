...
void micro_kernel_asm_r6c96_only_write(float* src_a, lvector float* src_b, lvector float* dst_c, 
    ...
    #else
void micro_kernel_asm_r6c96_multiAlpha(float* src_a, lvector float* src_b, lvector float* dst_c, 
    ...
    __asm__ __volatile__(
        ...
        "   |   SADD.M1         R9, %0, R9                  ;; R9 = src_A + 8*(1.5*K_buffer)                    \t\n"
        "   |   SMVAGA.M2       %4, OR9                     ;; OR9 = A矩阵两行的字数: K_buffer                   \t\n"
        "   |   SLDW		    *AR10, R51		            ;; (next next)load A[0][0,1]                        \t\n"
        "   |   VMOV.M1		    VR0, VR7				    ;; 初始化C寄存器                                     \t\n"
	    "   |	VMOV.M2		    VR0, VR8				    ;; 初始化C寄存器                                     \t\n"
	    "   |	VMOV.M3		    VR0, VR9				    ;; 初始化C寄存器                                     \t\n"
            // [0.9]
        ...
        "   |   SMVAGA.M1       R43, OR1                    ;; B/C_buffer一行多少个单字                          \t\n"
        "   |   SMVAGA.M2	    R9, AR13				    ;; AR13 = src_A + 8*(1.5*K_buffer)                  \t\n"
        "   |   SLDW		    *+AR10[OR8], R53		    ;; (next next) load A[1][0,1]                       \t\n"
        "   |   VMOV.M1		    VR0, VR10				    ;; 初始化C寄存器                                     \t\n"
	    "   |	VMOV.M2		    VR0, VR11				    ;; 初始化C寄存器                                     \t\n"
	    "   |	VMOV.M3		    VR0, VR12				    ;; 初始化C寄存器                                     \t\n"
            // [0.10]
        "       SADD.M1		    R21, %0, R21			    ;; R21 = src_A + K_data * 4 // A矩阵的首行末地址     \t\n"
        "   |   SADDA.M2        8, AR10, AR10               ;; AR10 += 8 Bytes                                  \t\n"
        "   |   SLDW		    *+AR10[OR9], R55		    ;; (next next) load A[2][0,1]                       \t\n"
        "   |   VMOV.M1		    VR0, VR13				    ;; 初始化C寄存器                                     \t\n"
	    "   |	VMOV.M2		    VR0, VR14				    ;; 初始化C寄存器                                     \t\n"
	    "   |	VMOV.M3		    VR0, VR15				    ;; 初始化C寄存器                                     \t\n"
            // [0.11]
        "       SLDW		    *AR13, R57		            ;; (next next) load A[3][0,1]               \t\n"
        "   |   VMOV.M1		    VR0, VR16				    ;; 初始化C寄存器                             \t\n"
	    "   |	VMOV.M2		    VR0, VR17				    ;; 初始化C寄存器                             \t\n"
        "   |   VMOVI.M3 	    0x3F8000003F800000,VR63	    ;; VR63 = 1.0                               \t\n" 
            // [1.0]
        "       SLDW            *+AR13[OR8], R59            ;; (next) load A[4][0,1]                    \t\n"
            // [1.1]
        "       SLDW            *+AR13[OR9], R61            ;; (next) load A[5][0,1]                    \t\n"
        "   |   SADDA.M2        8, AR13, AR13               ;; (next) AR13 += 8 Bytes                   \t\n"
            // [1.2]
        "       SBALE2          %6, %6, %6                  \t\n"
        "   |   SMVAAG.M1	    AR10, %0		            ;; R10 = src_A + 1              \t\n"
        "   |   VLDDW 		    *AR5++[OR2], VR37:VR36		;; load mat_C[0][0,1]                       \t\n"
        "   |   VLDW            *+AR5[32], VR38             ;; load mat_C[0][2]                         \t\n"
            // [1.3]
        "       SFMULS32.M1     %6, R51, R51                ;; (next) A[0] *= alpha                     \t\n"
        "   |   VLDDW 		    *AR4++[OR2], VR19:VR18		;; (next) load B[0][0,1]                    \t\n"
        "   |   VLDW            *+AR4[32], VR20             ;; (next) load B[0][2]                      \t\n"
            // [1.4]
        "       SFMULS32.M1     %6, R53, R53                ;; (next) A[1] *= alpha                     \t\n"
        "   |   VLDW            *AR5++[OR1], VR39           ;; load mat_C[1][0]                         \t\n"
        "   |   VLDDW 		    *+AR5[8], VR41:VR40		    ;; load mat_C[1][1,2]                       \t\n"    
            // [1.5]
        "       SFMULS32.M1     %6, R55, R55                ;; (next) A[2] *= alpha                     \t\n"
        "   |   VLDDW 		    *AR5++[OR2], VR43:VR42		;; load mat_C[2][0,1]                       \t\n"
        "   |   VLDW            *+AR5[32], VR44             ;; load mat_C[2][2]                         \t\n"
            // [1.6]
        "       SFMULS32.M1     %6, R57, R57                ;; (next) A[3] *= alpha                     \t\n"
        "   |   VLDW            *AR5++[OR1], VR45           ;; load mat_C[3][0]                         \t\n"
        "   |   VLDDW 		    *+AR5[8], VR47:VR46		    ;; load mat_C[3][1,2]                       \t\n"
            // [1.7]
        "       SFMULS32.M1     %6, R59, R59                ;; (next) A[4] *= alpha                     \t\n"
        "   |   SVBCAST.M2 	    R51, VR25                   ;; (next) broadcast A[0][0,1]               \t\n"
        "   |   VLDDW 		    *AR5++[OR2], VR49:VR48		;; load mat_C[4][0,1]                       \t\n"
        "   |   VLDW            *+AR5[32], VR50             ;; load mat_C[4][2]                         \t\n" 
            // [1.8]
        "       SFMULS32.M1     %6, R61, R61                ;; (next) A[5] *= alpha                     \t\n"
        "   |   SVBCAST.M2      R53, VR27                   ;; (next) broadcast _A[1][0,1]              \t\n"
        "   |   SLDW            *AR10, R51  		        ;; (next next) load A[0][0,1]               \t\n"
        "   |   VLDW            *AR5++[OR1], VR51           ;; load mat_C[5][0]                         \t\n"
        "   |   VLDDW 		    *+AR5[8], VR53:VR52		    ;; load mat_C[5][1,2]                       \t\n"
            // [1.9]
        "       SVBCAST.M2      R55, VR29                   ;; (next) broadcast A[2][0,1]               \t\n"
        "   |   VLDW            *AR4++[OR1], VR21           ;; (next) load B[1][0]                      \t\n"
        "   |   VLDDW           *+AR4[8], VR23:VR22	        ;; (next) load B[1][1,2]                    \t\n"
        "   |   SLDW            *+AR10[OR8], R53		    ;; (next next) load A[1][0,1]               \t\n"
            // [1.10]
        "       SVBCAST.M2      R57, VR31                   ;; (next) broadcast A[3][0,1]               \t\n"
        "   |   SLDW            *+AR10[OR9], R55		    ;; (next next) load A[2][0,1]               \t\n"
        "   |   SADDA.M1        8, AR10, AR10               ;; (next next) AR10 += 8 Bytes              \t\n"
            // [1.11]
        "       SVBCAST.M2      R59, VR33                   ;; (next) broadcast A[4][0,1]               \t\n"
        "   |   VBALE2          VR25, VR25, VR24            ;; (next) get A[0][0]                       \t\n"
        "   |   SLDW            *AR13, R57                  ;; (next next) load A[3][0,1]               \t\n"
        
        "loop_k_r6c96_multiAlpha:       \t\n"
        // -------------------------------------- MAIN LOOP --------------------------------------
            // [0]
        "       SVBCAST.M1      R61, VR35                   ;; broadcast A[5][0,1]                      \t\n"
        "   |   VBALE2          VR27, VR27, VR26            ;; get A[1][0]                              \t\n"
	    "   |   VFMULAS32.M1	VR18, VR24, VR0, VR0		;; VR0 += B[0][0] * A[0][0]                 \t\n"
	    "   |	VFMULAS32.M2	VR19, VR24, VR1, VR1		;; VR1 += B[0][1] * A[0][0]                 \t\n"
	    "   |	VFMULAS32.M3	VR20, VR24, VR2, VR2		;; VR2 += B[0][2] * A[0][0]                 \t\n"
        "   |   SLDW            *+AR13[OR8], R59            ;; (next) load A[4][0,1]                    \t\n"
            // [1]
        "       SLT			    %0, R21, R0                 ;; if R10 < src_A + K_data * 4  \t\n"
        "   |   SMVAAG.M1	    AR10, %0		            ;; R10 = src_A + 1              \t\n"
        "   |   VBALE2          VR29, VR29, VR28            ;; get A[2][0]                              \t\n"
        "   |   VFMULAS32.M1	VR18, VR26, VR3, VR3		;; VR3 += B[0][0] * A[1][0]                 \t\n"
	    "   |	VFMULAS32.M2	VR19, VR26, VR4, VR4		;; VR4 += B[0][1] * A[1][0]                 \t\n"
	    "   |	VFMULAS32.M3	VR20, VR26, VR5, VR5		;; VR5 += B[0][2] * A[1][0]                 \t\n"
        "   |   SLDW            *+AR13[OR9], R61            ;; (next) load A[5][0,1]                    \t\n"
        "   |   SADDA.M2        8, AR13, AR13               ;; (next) AR13 += 8 Bytes                   \r\n"
            // [2]
        "       VBALE2          VR31, VR31, VR30            ;; get A[3][0]                              \t\n"
        "   |   VFMULAS32.M1	VR18, VR28, VR6, VR6		;; VR6 += B[0][0] * A[2][0]                 \t\n"
	    "   |	VFMULAS32.M2	VR19, VR28, VR7, VR7		;; VR7 += B[0][1] * A[2][0]                 \t\n"
	    "   |	VFMULAS32.M3	VR20, VR28, VR8, VR8		;; VR8 += B[0][2] * A[2][0]                 \t\n"
            // [3]
        "       VBALE2          VR33, VR33, VR32            ;; get A[4][0]                              \t\n"
        "   |   VFMULAS32.M1	VR18, VR30, VR9, VR9		;; VR9 += B[0][0] * A[3][0]                 \t\n"
	    "   |	VFMULAS32.M2	VR19, VR30, VR10, VR10		;; VR10 += B[0][1] * A[3][0]                \t\n"
	    "   |	VFMULAS32.M3	VR20, VR30, VR11, VR11		;; VR11 += B[0][2] * A[3][0]                \t\n"
        "   |   SFMULS32.M1     %6, R51, R51                ;; (next) A[0] *= alpha                     \t\n"
        "   |   VLDDW 		    *AR4++[OR2], VR19:VR18		;; (next) load B[0][0,1]                    \t\n"
        "   |   VLDW            *+AR4[32], VR20             ;; (next) load B[0][2]                      \t\n"
            // [4]
        "       VBALE2          VR35, VR35, VR34            ;; get A[5][0]                              \t\n"
        "   |   VFMULAS32.M1	VR18, VR32, VR12, VR12		;; VR12 += B[0][0] * A[4][0]                \t\n"
	    "   |	VFMULAS32.M2	VR19, VR32, VR13, VR13		;; VR13 += B[0][1] * A[4][0]                \t\n"
	    "   |	VFMULAS32.M3	VR20, VR32, VR14, VR14		;; VR14 += B[0][2] * A[4][0]                \t\n"
        "   |   SFMULS32.M1     %6, R53, R53                ;; (next) A[1] *= alpha                     \t\n"
            // [5]
        "       VBALE2H         VR25, VR25, VR25            ;; get A[0][1]                              \t\n"
        "   |   VFMULAS32.M1	VR18, VR34, VR15, VR15		;; VR15 += B[0][0] * A[5][0]                \t\n"
	    "   |	VFMULAS32.M2	VR19, VR34, VR16, VR16		;; VR16 += B[0][1] * A[5][0]                \t\n"
	    "   |	VFMULAS32.M3	VR20, VR34, VR17, VR17		;; VR17 += B[0][2] * A[5][0]                \t\n"
        "   |   SFMULS32.M1     %6, R55, R55                ;; (next) A[2] *= alpha                     \t\n"
        "   |   [R0]    SBR		loop_k_r6c96_multiAlpha     ;; condjump to mainloop entrance            \t\n"
            // [6]
        "       VBALE2H         VR27, VR27, VR27            ;; get A[1][1]                              \t\n"
        "   |   VFMULAS32.M1	VR21, VR25, VR0, VR0		;; VR0 += B[1][0] * A[0][1]                 \t\n"
	    "   |	VFMULAS32.M2	VR22, VR25, VR1, VR1		;; VR1 += B[1][1] * A[0][1]                 \t\n"
	    "   |	VFMULAS32.M3	VR23, VR25, VR2, VR2		;; VR2 += B[1][2] * A[0][1]                 \t\n"
        "   |   SFMULS32.M1     %6, R57, R57                ;; (next) A[3] *= alpha                     \t\n"
            // [7]
        "       VBALE2H         VR29, VR29, VR29            ;; get A[2][1]                              \t\n"
        "   |   VFMULAS32.M1	VR21, VR27, VR3, VR3		;; VR3 += B[1][0] * A[1][1]                 \t\n"
        "   |	VFMULAS32.M2	VR22, VR27, VR4, VR4		;; VR4 += B[1][1] * A[1][1]                 \t\n"
        "   |	VFMULAS32.M3	VR23, VR27, VR5, VR5		;; VR5 += B[1][2] * A[1][1]                 \t\n"
        "   |   SFMULS32.M1     %6, R59, R59                ;; (next) A[4] *= alpha                     \t\n"
        "   |   SVBCAST.M2 	    R51, VR25                   ;; (next) broadcast A[0][0,1]               \t\n"
            // [8]
        "       VBALE2H         VR31, VR31, VR31            ;; get A[3][1]                              \t\n"
        "   |   VFMULAS32.M1	VR21, VR29, VR6, VR6		;; VR2 += B[1][0] * A[2][1]                 \t\n"
        "   |   VFMULAS32.M2	VR22, VR29, VR7, VR7		;; VR7 += B[1][1] * A[2][1]                 \t\n"
        "   |   VFMULAS32.M3	VR23, VR29, VR8, VR8		;; VR8 += B[1][2] * A[2][1]                 \t\n"
        "   |   SFMULS32.M1     %6, R61, R61                ;; (next) A[5] *= alpha                     \t\n"
        "   |   SVBCAST.M2      R53, VR27                   ;; (next) broadcast _A[1][0,1]              \t\n"
        "   |   SLDW            *AR10, R51  		        ;; (next next) load A[0][0,1]               \t\n"
            // [9]
        "       VBALE2H         VR33, VR33, VR33            ;; get A[4][1]                              \t\n"
        "   |   VFMULAS32.M1	VR21, VR31, VR9, VR9		;; VR9 += B[1][0] * A[3][1]                 \t\n"
        "   |	VFMULAS32.M2	VR22, VR31, VR10, VR10		;; VR10 += B[1][1] * A[3][1]                \t\n"
        "   |	VFMULAS32.M3	VR23, VR31, VR11, VR11		;; VR11 += B[1][2] * A[3][1]                \t\n"
        "   |   SVBCAST.M2      R55, VR29                   ;; (next) broadcast A[2][0,1]               \t\n"
        "   |   VLDW            *AR4++[OR1], VR21           ;; (next) load B[1][0]                      \t\n"
        "   |   VLDDW           *+AR4[8], VR23:VR22	        ;; (next) load B[1][1,2]                    \t\n"
        "   |   SLDW            *+AR10[OR8], R53		    ;; (next next) load A[1][0,1]               \t\n"
            // [10]
        "       VBALE2H         VR35, VR35, VR35            ;; get A[5][1]                              \t\n"
        "   |   VFMULAS32.M1	VR21, VR33, VR12, VR12		;; VR12 += B[1][0] * A[4][1]                \t\n"
	    "   |	VFMULAS32.M2	VR22, VR33, VR13, VR13		;; VR13 += B[1][1] * A[4][1]                \t\n"
	    "   |	VFMULAS32.M3	VR23, VR33, VR14, VR14		;; VR14 += B[1][2] * A[4][1]                \t\n"
        "   |   SVBCAST.M2      R57, VR31                   ;; (next) broadcast A[3][0,1]               \t\n"
        "   |   SLDW            *+AR10[OR9], R55		    ;; (next next) load A[2][0,1]               \t\n"
        "   |   SADDA.M1        8, AR10, AR10               ;; (next next) AR10 += 8 Bytes              \r\n"
            // [11]
        "       VFMULAS32.M1	VR21, VR35, VR15, VR15		;; VR15 += B[1][0] * A[5][1]                \t\n"
	    "   |	VFMULAS32.M2	VR22, VR35, VR16, VR16		;; VR16 += B[1][1] * A[5][1]                \t\n"
	    "   |	VFMULAS32.M3	VR23, VR35, VR17, VR17		;; VR17 += B[1][2] * A[5][1]                \t\n"
        "   |   SVBCAST.M2      R59, VR33                   ;; (next) broadcast A[4][0,1]               \t\n"
        "   |   VBALE2          VR25, VR25, VR24            ;; (next) get A[0][0]                       \t\n"
        "   |   SLDW            *AR13, R57                  ;; (next next) load A[3][0,1]               \t\n"
        // -------------------------------------- write back --------------------------------------
...
void micro_kernel_asm_r6c96_multiAlpha_only_write(float* src_a, lvector float* src_b, lvector float* dst_c, 
                        const indexType K_data, const indexType K_buffer, const indexType N_buffer, const float alpha){
    __asm__ __volatile__(
            // [-3]
        "       SSHFLR          1, %4, R7                   ;; R7 = K_buffer/2  // A矩阵一行的字数               \t\n"
        ...
        "loop_k_r6c96_multiAlpha_only_write:       \t\n"
        // -------------------------------------- MAIN LOOP --------------------------------------
            // [0]
        "       SVBCAST.M1      R61, VR35                   ;; broadcast A[5][0,1]                      \t\n"
        "   |   VBALE2          VR27, VR27, VR26            ;; get A[1][0]                              \t\n"
	    ...
	    "   |	VFMULAS32.M3	VR20, VR34, VR17, VR17		;; VR17 += B[0][2] * A[5][0]                \t\n"
        "   |   SFMULS32.M1     %6, R55, R55                ;; (next) A[2] *= alpha                     \t\n"
        "   |   [R0]    SBR		loop_k_r6c96_multiAlpha_only_write     ;; condjump to mainloop entrance            \t\n"
            // [6]
        "       VBALE2H         VR27, VR27, VR27            ;; get A[1][1]                              \t\n"
        "   |   VFMULAS32.M1	VR21, VR25, VR0, VR0		;; VR0 += B[1][0] * A[0][1]                 \t\n"
	    "   |	VFMULAS32.M2	VR22, VR25, VR1, VR1		;; VR1 += B[1][1] * A[0][1]                 \t\n"
...
// traverse order: K_G -> N_G -> M_A -> K_A -> N_S
        ...
        for(long n_o=0; n_o<N; n_o+=N_G){
            ...
            for(long m_o=m_start; m_o<M; m_o+=m_step){
                ...
                long k_i = 0;
                    ...
                    #else
                        micro_kernel_asm_r6c96_multiAlpha_only_write(spm_B[cnt_b], spm_A[cnt_a], spm_C[cnt_c_1] + n_s*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                        micro_kernel_asm_r6c96_multiAlpha_only_write(spm_B[cnt_b] + N_S_half*K_A, spm_A[cnt_a], spm_C[cnt_c_1] + (n_s + N_S_half)*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                    #endif
                #endif
                #ifdef use_r6c128
                    #ifndef enable_alpha
                    ...
                    #else
                        micro_kernel_asm_r6c128_multiAlpha_only_write(spm_B[cnt_b], spm_A[cnt_a], spm_C[cnt_c_1] + n_s*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                        micro_kernel_asm_r6c128_multiAlpha_only_write(spm_B[cnt_b] + N_S_half*K_A, spm_A[cnt_a], spm_C[cnt_c_1] + (n_s + N_S_half)*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                    #endif
                #endif
                        cnt_b = cnt_b_1;
                    }
                ...
                for(k_i=K_A; k_i<K_G_cur; k_i+=K_A){
                    ...
                    #else
                        micro_kernel_asm_r6c96_multiAlpha(spm_B[cnt_b], spm_A[cnt_a], spm_C[cnt_c_1] + n_s*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                        micro_kernel_asm_r6c96_multiAlpha(spm_B[cnt_b] + N_S_half*K_A, spm_A[cnt_a], spm_C[cnt_c_1] + (n_s + N_S_half)*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                    #endif
                #endif
                #ifdef use_r6c128
                    #ifndef enable_alpha
                    ...
                    #else
                        micro_kernel_asm_r6c128_multiAlpha(spm_B[cnt_b], spm_A[cnt_a], spm_C[cnt_c_1] + n_s*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                        micro_kernel_asm_r6c128_multiAlpha(spm_B[cnt_b] + N_S_half*K_A, spm_A[cnt_a], spm_C[cnt_c_1] + (n_s + N_S_half)*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                    #endif
                #endif
                        cnt_b = cnt_b_1;
                    }
                ...
                for(int n_a=0; n_a<N_G_cur; n_a+=32){
                    for(int m_a=0; m_a<M_A_cur; m_a+=32){
                        micro_kernel_float_32x32(&spm_C[cnt_c_1][OFFSET(n_a, m_a>>5, M_A>>5)], &spm_C[cnt_c][OFFSET(m_a, n_a>>5, N_A_buffer>>5)], spm_D, N_A_buffer, M_A);
                    }
                }
                ...
                if(n_o_next != -1){ // m_o到头了，n_o,k_o没到头
        ...
        for(long n_o=0; n_o<N; n_o+=N_G){
            ...
            for(long m_o=m_start; m_o<M; m_o+=m_step){
                ...
                long k_i = 0;
                    ...
                    #else
                        micro_kernel_asm_r6c96_multiAlpha_only_write(spm_B[cnt_b], spm_A[1], spm_C[cnt_c_1] + n_s*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                        micro_kernel_asm_r6c96_multiAlpha_only_write(spm_B[cnt_b] + N_S_half*K_A, spm_A[1], spm_C[cnt_c_1] + (n_s + N_S_half)*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                    #endif
                #endif
                #ifdef use_r6c128
                    #ifndef enable_alpha
                    ...
                    #else
                        micro_kernel_asm_r6c128_multiAlpha_only_write(spm_B[cnt_b], spm_A[1], spm_C[cnt_c_1] + n_s*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                        micro_kernel_asm_r6c128_multiAlpha_only_write(spm_B[cnt_b] + N_S_half*K_A, spm_A[1], spm_C[cnt_c_1] + (n_s + N_S_half)*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                    #endif
                #endif
                        cnt_b = cnt_b_1;
                    }
                ...
                for(k_i=K_A; k_i<K_G_cur; k_i+=K_A){
                    ...
                    #else
                        micro_kernel_asm_r6c96_multiAlpha(spm_B[cnt_b], spm_A[1], spm_C[cnt_c_1] + n_s*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                        micro_kernel_asm_r6c96_multiAlpha(spm_B[cnt_b] + N_S_half*K_A, spm_A[1], spm_C[cnt_c_1] + (n_s + N_S_half)*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                    #endif
                #endif
                #ifdef use_r6c128
                    #ifndef enable_alpha
                    ...
                    #else
                        micro_kernel_asm_r6c128_multiAlpha(spm_B[cnt_b], spm_A[1], spm_C[cnt_c_1] + n_s*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                        micro_kernel_asm_r6c128_multiAlpha(spm_B[cnt_b] + N_S_half*K_A, spm_A[1], spm_C[cnt_c_1] + (n_s + N_S_half)*(M_A>>5), K_A_cur, K_A, M_A, alpha);
                    #endif
                #endif
                        cnt_b = cnt_b_1;
                    }
...
__global__ void irr_thinB_sgemm_TN(unlong M, unlong N, unlong K, unlong bid_main, unlong bid_T, float* A, float* B, float* C, float alpha, float beta){
    ...
    if(tid >= n_cores_for_compute){
        ...
        irr_thinB_sgemm_TN_mainbody(M, N, K, bid_main, n_cores_for_compute, A, B, C, alpha, beta);
    }
}
...
__global__ void irr_thinB_sgemm_NN(unlong M, unlong N, unlong K, unlong bid_main, unlong bid_T, float* A, float* B, float* C, float alpha, float beta){
    ...
    if(tid >= n_cores_for_compute){
        ...
        irr_thinB_sgemm_NN_mainbody(M, N, K, bid_main, n_cores_for_compute, A, B, C, alpha, beta);
    }
}
